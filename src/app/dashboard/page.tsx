    ...
    <div className="min-h-screen bg-[#050506] text-zinc-100 relative pb-20 sm:pb-0">
      ...
      <header className="border-b border-white/[0.04] bg-[#050506]/80 backdrop-blur-2xl sticky top-0 z-50 safe-top">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-[15px] font-extrabold tracking-tight">RADAR<span className="text-gradient">MIX</span> <span className="text-orange-500/80 text-[11px] font-semibold ml-0.5">IA</span></h1>
          </div>
          {/* Desktop nav links */}
          <div className="hidden md:flex items-center gap-4">
            <div className="h-4 w-px bg-zinc-800"></div>
            <Link href="/chat" className="flex items-center gap-1.5 text-[12px] text-zinc-500 hover:text-orange-400 transition-colors font-medium">
              <span className="w-1.5 h-1.5 rounded-full bg-orange-500 pulse-dot"></span>Radar IA Chat
            </Link>
            <Link href="/marketplace" className="text-[12px] text-zinc-500 hover:text-orange-400 transition-colors font-medium">🛒 Connect</Link>
            <Link href="/ranking" className="text-[12px] text-zinc-500 hover:text-orange-400 transition-colors font-medium">🏆 Ranking</Link>
            <Link href="/checklist" className="text-[12px] text-zinc-500 hover:text-orange-400 transition-colors font-medium">📋 Checklist</Link>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-[11px] text-zinc-700 hidden sm:block mono truncate max-w-[140px]">{user.email}</span>
            <LogoutButton />
          </div>
        </div>
      </header>
      ...
      <nav className="bottom-nav sm:hidden safe-bottom">
        <div className="grid grid-cols-5 max-w-lg mx-auto">
          ...
          <Link href="/marketplace" className="bottom-nav-item">
            <span className="nav-icon">🛒</span>
            <span>Connect</span>
          </Link>
          <Link href="/ranking" className="bottom-nav-item">
            <span className="nav-icon">🏆</span>
            <span>Ranking</span>
          </Link>
          <Link href="/checklist" className="bottom-nav-item">
            <span className="nav-icon">📋</span>
            <span>Checklist</span>
          </Link>
        </div>
